...
( function() {
	...
	var bbcodeMap = { b: 'strong', u: 'u', i: 'em', justify:'div', left:'div', right:'div', center:'div', block:'div',bgcolor: 'span', color: 'span', size: 'span', quote: 'blockquote', code: 'code', youtube: 'iframe', url: 'a', email: 'span', img: 'span', '*': 'li', list: 'ol' , s: 'span'},
			convertMap = { strong: 'b', b: 'b', u: 'u', em: 'i', i: 'i', code: 'code',iframe: 'youtube', li: '*' },
			tagnameMap = { strong: 'b', em: 'i', u: 'u', li: '*', ul: 'list', ol: 'list', code: 'code',iframe: 'youtube', a: 'link', img: 'img', blockquote: 'quote' },
			stylesMap = { color: 'color', size: 'font-size', bgcolor: 'background-color',justify:"text-align",left:"text-align",right:"text-align",center:"text-align",block:"text-align" },
			attributesMap = { url: 'href', email: 'mailhref',code: 'class', quote: 'cite', list: 'listType' };
	...
	var BBCodeWriter = CKEDITOR.tools.createClass( {
		...
		proto: {
          ...
          this.write(' allowfullscreen=', val);
        } else if (name == 'frameborder') {
          this.write(' frameborder=', val);
        }*/
			},
	...
	CKEDITOR.plugins.add( 'bbcode', {
		...
		init: function( editor ) {
					...
					div: function( element ) {
						...
						if ( ( bbcode = element.attributes.bbcode ) ) {
							if ( bbcode == 'left' ) {
								...
								element.name = 'center';
							} else if ( bbcode == 'block' ) {
								element.name = 'block';
							} else if ( bbcode == 'justify' ) {
								element.name = 'justify';
							}

							element.attributes.style = 'text-align:' + bbcode;
			...
			editor.dataProcessor.htmlFilter.addRules( {
              ...
              // element.attributes.frameborder = attributes.frameborder;
              // element.attributes.allowfullscreen = attributes.allowfullscreen;
						} else if ( tagName == 'img' ) {
							element.isEmpty = 0;
					...
					filters.push( function( element ) {
						...
						// Styled span could be either size or color.
							...
							if ( element.getStyle( 'font-size' ) )
								...
								name = 'bgcolor';
							else if ( element.getStyle( 'backgroundColor' ) )
								name = 'bgcolor';
						} else if ( name == 'img' ) {
							var src = element.data( 'cke-saved-src' ) || element.getAttribute( 'src' );
							if ( src && src.indexOf( editor.config.smiley_path ) === 0 )
								name = 'smiley';
						}else if ( name == 'div' && element.getStyle( 'text-align' )) {
							align = element.getStyle( 'text-align' );
							if ( align )
								name = align;
						}else if ( name == 'div' && element.getStyle( 'textAlign' )) {
